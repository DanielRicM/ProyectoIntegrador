...
public class SongFactory implements ObjFactory<Identifiable> {
    ...
    public String toQuery(Identifiable object) {
        Song song = (Song) object;
        return String.valueOf(song.getId()) + ", '" + song.getName() + "', '" + song.getAuthor() + "', '" + song.getAlbum() + "'";
    }

    @Override
    public String toUpdateQuery(Identifiable object) {
        Song song = (Song) object;
        return "UPDATE song SET name = '" + song.getName() + "', author = '" + song.getAuthor() + "', album = '" + song.getAlbum() + "' WHERE id = " + song.getId();
    }
    ...
    public String createTableQuery() {
        return "CREATE TABLE IF NOT EXISTS song (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, author TEXT, album TEXT)";
    }
